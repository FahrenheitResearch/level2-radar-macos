...
std::vector<uint8_t> decodeArchiveBytesImpl(const std::vector<uint8_t>& fileData,
    ...
    if (fileData.size() < kArchiveHeaderSize) return combined;
    ...
    if (!blocks.empty()) {
        if (totalBlocksOut) *totalBlocksOut = (int)blocks.size();
        for (int i = 0; i < (int)blocks.size(); i++) {
            const auto& block = blocks[i];
            std::vector<uint8_t> decoded;
            if (block.compressed) {
                decompressBZ2Stream(archiveData + block.offset, block.size, decoded, nullptr);
            } else {
            ...
            if (cb) cb(i + 1, (int)blocks.size());
        }
    }

    if (combined.empty()) {
        const auto fallbackBlocks = decompressLegacyStreams(archiveData, archiveSize);
        if (totalBlocksOut) *totalBlocksOut = (int)fallbackBlocks.size();
        for (int i = 0; i < (int)fallbackBlocks.size(); i++) {
            if (!appendBytes(combined, fallbackBlocks[i].data(), fallbackBlocks[i].size())) {
                combined.clear();
                break;
            }
            if (cb) cb(i + 1, (int)fallbackBlocks.size());
        }
    }
