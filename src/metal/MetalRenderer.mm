...
static MTLSize threadsPerGroup1D(id<MTLComputePipelineState> pso) {
    const NSUInteger width = std::max<NSUInteger>(1, std::min<NSUInteger>(pso.threadExecutionWidth,
                                                                           pso.maxTotalThreadsPerThreadgroup));
    return MTLSizeMake(width, 1, 1);
}

static MTLSize threadsPerGroup2D(id<MTLComputePipelineState> pso) {
    const NSUInteger width = std::max<NSUInteger>(1, std::min<NSUInteger>(pso.threadExecutionWidth,
                                                                           pso.maxTotalThreadsPerThreadgroup));
    NSUInteger height = std::max<NSUInteger>(1, pso.maxTotalThreadsPerThreadgroup / width);
    height = std::min<NSUInteger>(height, 16);
    while (width * height > pso.maxTotalThreadsPerThreadgroup && height > 1)
        --height;
    return MTLSizeMake(width, height, 1);
}

static void dispatchThreads1D(id<MTLComputeCommandEncoder> enc,
                              id<MTLComputePipelineState> pso,
                              NSUInteger width) {
    const MTLSize threadsPerGroup = threadsPerGroup1D(pso);
    [enc dispatchThreads:MTLSizeMake(width, 1, 1) threadsPerThreadgroup:threadsPerGroup];
}

static void dispatchThreads2D(id<MTLComputeCommandEncoder> enc,
                              id<MTLComputePipelineState> pso,
                              NSUInteger width,
                              NSUInteger height) {
    const MTLSize threadsPerGroup = threadsPerGroup2D(pso);
    [enc dispatchThreads:MTLSizeMake(width, height, 1) threadsPerThreadgroup:threadsPerGroup];
}

static void dispatchThreads3D(id<MTLComputeCommandEncoder> enc,
                              id<MTLComputePipelineState> pso,
                              NSUInteger width,
                              NSUInteger height,
                              NSUInteger depth) {
    const MTLSize threadsPerGroup = threadsPerGroup2D(pso);
    [enc dispatchThreads:MTLSizeMake(width, height, depth) threadsPerThreadgroup:threadsPerGroup];
}
...
MetalRenderer::MetalRenderer()
    ...
    , _volumeAzimuthBuffer(nil)
    , _volumeGateBuffer(nil)
{
...
void MetalRenderer::clearOutputBuffer(const GpuViewport& vp, id<MTLBuffer> output) {
    if (_clearPSO && output) {
        ...
        const struct { int x; int y; } dims = {vp.width, vp.height};
        const uint32_t color = kBackgroundColor;
        [enc setBytes:&dims length:sizeof(dims) atIndex:1];
        [enc setBytes:&color length:sizeof(color) atIndex:2];
        dispatchThreads2D(enc, _clearPSO, (NSUInteger)vp.width, (NSUInteger)vp.height);
        [enc endEncoding];
        [cmdBuf commit];
        return;
    }
...
void MetalRenderer::renderNative(const GpuViewport& vp,
    ...
    id<MTLCommandBuffer> cmdBuf = [_commandQueue commandBuffer];
    {
        ...
        [blit fillBuffer:_forwardAccumBuffer range:NSMakeRange(0, pixel_count * sizeof(uint32_t)) value:0xFF];
        [blit fillBuffer:_forwardColorBuffer range:NSMakeRange(0, pixel_count * sizeof(uint32_t)) value:0x00];
        [blit endEncoding];
    }
    ...
    for (int i = 0; i < num_stations && i < MAX_STATIONS; i++) {
        ...
        dispatchThreads2D(enc, _forwardRenderPSO,
                          (NSUInteger)st.info.num_radials,
                          (NSUInteger)st.info.num_gates[product]);
        [enc endEncoding];
    }
    ...
    // Resolve: depth+color -> final RGBA output (same command buffer, no wait between)
    {
        ...
        dispatchThreads2D(enc, _forwardResolvePSO, (NSUInteger)vp.width, (NSUInteger)vp.height);
        [enc endEncoding];
    }
...
void MetalRenderer::renderSingleStation(const GpuViewport& vp, int station_idx,
    ...
    dispatchThreads2D(enc, _singleStationPSO, (NSUInteger)vp.width, (NSUInteger)vp.height);
    [enc endEncoding];
    [cmdBuf commit];
...
void MetalRenderer::forwardRenderStation(const GpuViewport& vp, int station_idx,
    ...
    // Velocity is visually sensitive to forward-render wedge ties.
    // Prefer the inverse-mapped path until we have a packed atomic resolve
    // like the CUDA renderer.
    if (product == PROD_VEL) {
    ...
    // Pass 1: Forward render (one thread per radial/gate pair)
    {
        ...
        dispatchThreads2D(enc, _forwardRenderPSO,
                          (NSUInteger)station.info.num_radials,
                          (NSUInteger)station.info.num_gates[product]);
        [enc endEncoding];
    }
...
void MetalRenderer::buildSpatialGridGpu(const GpuStationInfo* h_stations, int num_stations,
    ...
    dispatchThreads1D(enc, _buildGridPSO, (NSUInteger)num_stations);
    [enc endEncoding];
    [cmdBuf commit];
    [cmdBuf waitUntilCompleted];
...
void MetalRenderer::buildVolume(int station_idx, int product,
    ...
    for (int s = 0; s < num_sweeps && (int)h_sweeps.size() < 32; s++) {
        ...
        sw.azimuth_buffer_offset = (uint32_t)total_azimuths;
        sw.gate_buffer_offset = (uint32_t)total_gates;
        h_sweeps.push_back(sw);

        total_azimuths += (size_t)info.num_radials;
        total_gates += (size_t)info.num_radials * (size_t)info.num_gates[product];
    }

    int count = (int)h_sweeps.size();
    if (count <= 0 || total_azimuths == 0 || total_gates == 0) return;

    const size_t azimuth_bytes = total_azimuths * sizeof(float);
    if (!_volumeAzimuthBuffer || _volumeAzimuthBuffer.length < azimuth_bytes) {
        _volumeAzimuthBuffer = [_device newBufferWithLength:azimuth_bytes
                                                    options:MTLResourceStorageModeShared];
    }

    const size_t gate_bytes = total_gates * sizeof(uint16_t);
    if (!_volumeGateBuffer || _volumeGateBuffer.length < gate_bytes) {
        _volumeGateBuffer = [_device newBufferWithLength:gate_bytes
                                                 options:MTLResourceStorageModeShared];
    }

    float* azimuth_dst = (float*)_volumeAzimuthBuffer.contents;
    uint16_t* gate_dst = (uint16_t*)_volumeGateBuffer.contents;

    size_t azimuth_cursor = 0;
    size_t gate_cursor = 0;
    for (int s = 0; s < count; s++) {
        const SweepDesc& sw = h_sweeps[s];
        const size_t azimuth_count = (size_t)sw.num_radials;
        const size_t gate_count = (size_t)sw.num_radials * (size_t)sw.num_gates;

        id<MTLBuffer> azimuth_src = azimuths_per_sweep[s];
        id<MTLBuffer> gate_src = gates_per_sweep[s];
        if (!azimuth_src || !gate_src ||
            azimuth_src.length < azimuth_count * sizeof(float) ||
            gate_src.length < gate_count * sizeof(uint16_t)) {
            return;
        }

        memcpy(azimuth_dst + azimuth_cursor, azimuth_src.contents,
               azimuth_count * sizeof(float));
        memcpy(gate_dst + gate_cursor, gate_src.contents,
               gate_count * sizeof(uint16_t));

        azimuth_cursor += azimuth_count;
        gate_cursor += gate_count;
    }
    ...
    // Pass 1: Build volume from sweep data
    {
        ...
        [enc setBuffer:_volumeAzimuthBuffer offset:0 atIndex:2];
        [enc setBuffer:_volumeGateBuffer offset:0 atIndex:3];
        [enc setBytes:&count length:sizeof(count) atIndex:4];
        [enc setBytes:&product length:sizeof(product) atIndex:5];

        dispatchThreads3D(enc, _buildVolumePSO, (NSUInteger)VOL_XY, (NSUInteger)VOL_XY, (NSUInteger)VOL_Z);
        [enc endEncoding];
    }
    ...
    for (int pass = 0; pass < kVolumeSmoothPasses; ++pass) {
        ...
        dispatchThreads3D(enc, _smoothVolumePSO, (NSUInteger)VOL_XY, (NSUInteger)VOL_XY, (NSUInteger)VOL_Z);
        [enc endEncoding];
    }
...
void MetalRenderer::renderVolume(const Camera3D& cam, int width, int height,
    ...
    dispatchThreads2D(enc, _rayMarchPSO, (NSUInteger)width, (NSUInteger)height);
    [enc endEncoding];
    [cmdBuf commit];
...
void MetalRenderer::renderCrossSection(int station_idx, int product, float dbz_min,
                                        ...
                                        id<MTLBuffer> output) {
    (void)station_idx;
    if (!_volumeReady || !_crossSectionPSO || !_sweepDescBuffer ||
        !_volumeAzimuthBuffer || !_volumeGateBuffer || _volumeSweepCount <= 0) {
        return;
    }
    ...
    [enc setBuffer:_volumeAzimuthBuffer offset:0 atIndex:3];
    [enc setBuffer:_volumeGateBuffer offset:0 atIndex:4];
    [enc setBytes:&_volumeSweepCount length:sizeof(_volumeSweepCount) atIndex:5];
    [enc setBuffer:_colorTableBuffer offset:0 atIndex:6];

    dispatchThreads2D(enc, _crossSectionPSO, (NSUInteger)width, (NSUInteger)height);
    [enc endEncoding];
    [cmdBuf commit];
